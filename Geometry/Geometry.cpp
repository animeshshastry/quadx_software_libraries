...
Rotation &Rotation::FromQuaternions(float qw, float qx, float qy, float qz)
{
    float xx      = qx * qx;
    float xy      = qx * qy;
    float xz      = qx * qz;
...
Rotation &Rotation::FromOrthoVectors(Point x, Point y, Point z)
{
    (*this)(0,0) = x(0);
    (*this)(1,0) = x(1);
    (*this)(2,0) = x(2);
...
Rotation &Rotation::FromStereo(Matrix<2,1> s, float psi)
{
    float s1 = s(0);
    float s2 = s(1);

    Matrix<3, 1> RTe3 = { -2.0 * s1, -2.0 * s2, (1.0 - s1 * s1 - s2 * s2)};
    RTe3 = RTe3 * (1.0f / (1.0 + s1 * s1 + s2 * s2));

    float b3x = -RTe3(0);
    float b3y = -RTe3(1);
    float b3z = RTe3(2);
    Matrix<3,3> R_tilt = {  b3z+b3y*b3y/(1+b3z),    -b3x*b3y/(1+b3z),   b3x,
                            -b3x*b3y/(1+b3z),       1-b3y*b3y/(1+b3z),  b3y,
                            -b3x,                   -b3y,               b3z};

    Matrix<3,3> R_psi = {   cos(psi),  -sin(psi),   0,
                            sin(psi),   cos(psi),   0,
                            0,          0,          1};
    (*this) = R_psi*R_tilt;
//    (*this) = R_tilt*R_psi;
